...
const { parsed } = config({
	path: [
		join(path, `.env.${env.NODE_ENV ?? 'development'}`),
		join(path, '.env')
	]
})
...
export default class DragonEnv {
	...
	async build() {
		...
		const {
			rows: [data]
		} = await this.client.query('SELECT * FROM acct.get_company_data();')
		...
		for (const entry of Object.entries(
			Object.assign({}, { ...data, ...parsed, RDS_CERT: ca })
		).sort())
			envLocal += `${entry[0]} = '${entry[1]}'\n`
