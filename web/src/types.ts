...
type ExcludeKeyCustomListener = Exclude<
	keyof HTMLElementEventMap,
	'custom-listener'
>

type CustomListener = {
	[Key in ExcludeKeyCustomListener]: {
		listener: (event: HTMLElementEventMap[Key]) => void
		type: Key
	}
}[ExcludeKeyCustomListener]

interface RouterAppend {
	routeTemplates: HTMLTemplateElement[]
	siblingRouteTemplateId?: string
	type?: 'layout' | 'route'
}
interface RouterRequest {
	url: { id: 'fallbackId' | 'requestedId' } | string
}
interface RouterSet {
...
export type CustomElementEventMap = {
	[Key in keyof CustomElementEventDetail]: CustomEvent<
		CustomElementEventDetail[Key]
	>
}
