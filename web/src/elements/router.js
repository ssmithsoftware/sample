...
export default class RouterElement extends CustomElement {
  ...
  static observedAttributes = ['data-fallback-id', 'data-route-id', 'data-template-id']

  #elements = { div: document.createElement('div'), slot: document.createElement('slot'), template: document.createElement('template') }
  ...
  connectedCallback () {
    ...
    this.disconnectedCallbacks.push(() => window.removeEventListener('popstate', onPopState))
  ...
  #dispatchRouterSet (success) {
    queueMicrotask(() => {
      ...
      const detail = { fallbackId: this.fallbackId, requestedId: this.requestedId, routeId: this.routeId, success }

      for (const listener of this.#listeners) {
        if (listener.isConnected) listener.dispatchEvent(new CustomEvent('router-set', { detail }))
  ...
  #getDefaultTemplate (id, node) {
    const defaultTemplate = (node instanceof HTMLTemplateElement ? node.content : node).getElementById(id) ?? this.#elements.template.cloneNode()

    if (!(defaultTemplate instanceof HTMLTemplateElement)) throw new Error(`${elementMissing({ selectors: `template#${id}`, shadowRoot: node instanceof ShadowRoot })}${node instanceof Document ? ' in document' : node instanceof HTMLTemplateElement ? ` in template#${node.id}` : ''}`)
  ...
  #getSlotData (slot, clonedSlot) {
    ...
    if (!slotData) {
      ...
      if (!parentElement) throw new Error('cloned slot parent element does not exist')

      if (slot.hasChildNodes()) {
        nodes.push(...slot.assignedNodes({ flatten: true }).map((node) => node.cloneNode(true)))
  ...
  #onClick (event) {
    ...
    if (anchor && anchor.target !== '_blank') {
      ...
      this.#setRoute(anchor.pathname, anchor.search, () => history.pushState(history.state, '', anchor.href))
    }
  }

  /** @param {CustomElementEventMap['custom-listener']} event */
  #onCustomListener (event) {
    if (event.target instanceof HTMLElement && event.detail?.type === 'router-set') {
      event.target.addEventListener(event.detail.type, event.detail.listener)

      if (!this.#listeners.has(event.target)) this.#listeners.add(event.target)
    }
  }
  ...
  #onFallbackIdChanged (id) {
    if (this.#routesTemplate) {
      this.#fallbackTemplate = this.#getDefaultTemplate(this.#getRouteTemplateId(id), this.#routesTemplate)

      if (!this.#routesTemplate.content.contains(this.#fallbackTemplate)) this.#routesTemplate.content.insertBefore(this.#elements.div.cloneNode(), this.#rootTemplate).appendChild(this.#elements.template.cloneNode()).parentElement?.appendChild(this.#fallbackTemplate)
    }
  }
  ...
  #onRouteIdChanged (id) {
    if (this.fallbackId && this.#fallbackTemplate && this.#routesTemplate) {
      const routeTemplate = this.#routesTemplate.content.getElementById(this.#getRouteTemplateId(id))

      if (routeTemplate instanceof HTMLTemplateElement) this.#setRouteTemplate(routeTemplate)
      else this.routeId = id.startsWith(this.fallbackId) ? this.#getRouteParentId(id) : this.fallbackId
    } else this.#setRouteIds()
  }

  /** @param {CustomElementEventMap['router-append']} event */
  #onRouterAppend (event) {
    if (!this.#routesTemplate) throw new Error(elementMissing({ selectors: `template#${this.templateId}` }))

    const { routeTemplates, siblingRouteTemplateId = location.pathname, type = 'route' } = event.detail ?? {}

    let siblingRouteTemplate = this.#routesTemplate.content.getElementById(this.#getRouteTemplateId(siblingRouteTemplateId))
    if (!(siblingRouteTemplate instanceof HTMLTemplateElement)) throw new Error(`${elementMissing({ selectors: `template#${siblingRouteTemplateId}` })} in template#${this.templateId}`)

    for (const routeTemplate of routeTemplates) {
      if (!siblingRouteTemplate.parentElement) throw new Error('sibling route template parent element does not exist')

      this.#getRouteTemplateId(routeTemplate.id)

      if (type === 'route') siblingRouteTemplate = siblingRouteTemplate.parentElement.insertBefore(routeTemplate, siblingRouteTemplate.nextElementSibling)
      else {
        const clonedParentElement = this.#elements.div.cloneNode()
        if (!(clonedParentElement instanceof HTMLElement)) throw new Error('parent element is not an HTMLElement')

        const clonedRouteTemplate = this.#elements.template.cloneNode()
        if (!(clonedRouteTemplate instanceof HTMLTemplateElement)) throw new Error('route template is not an HTMLTemplateElement')

        clonedRouteTemplate.id = routeTemplate.id
        routeTemplate.removeAttribute('id')

        siblingRouteTemplate.parentElement.insertBefore(clonedParentElement, siblingRouteTemplate).append(routeTemplate, clonedRouteTemplate)
      }
    }
  }
  ...
  #onRouterRequest (event) {
    ...
    else if (typeof event.detail?.url === 'object' && this.#urlIds.includes(event.detail.url.id)) {
      const urlId = this[event.detail.url.id]

      if (urlId) urlString = urlId.concat(location.search)
    }

    if (typeof urlString !== 'string') throw new Error(`"${urlString}" is not a string`)
     ...
     */
    queueMicrotask(() => this.#setRoute(url.pathname, url.search, () => history[`${event.type.endsWith('push') ? 'push' : 'replace'}State`](history.state, '', url.href)))
  }

  /** @param {Event} event */
  #onSlotChange (event) {
    if (!(event.target instanceof HTMLSlotElement)) throw new Error('event target is not an HTMLSlotElement')
    ...
    for (const node of nodes) {
      ...
      slot.parentElement.insertBefore(clonedNode, this.#getReferenceNode(slot.nodes[0]))

      if (node instanceof Element && clonedNode instanceof Element) {
        clonedNode.removeAttribute('slot')

        if (node instanceof HTMLSlotElement && clonedNode instanceof HTMLSlotElement) {
          if (event.target === node) this.#slots.delete(node)

          clonedNodes.push(this.#getSlotData(node, clonedNode))
        } else {
          const [nodeSlots, clonedNodeSlots] = [node.querySelectorAll('slot'), clonedNode.querySelectorAll('slot')]

          for (let index = 0; index < nodeSlots.length; index++) {
            const nodeSlot = nodeSlots[index]
            const clonedNodeSlot = clonedNodeSlots[index]

            if (nodeSlot instanceof HTMLSlotElement && clonedNodeSlot instanceof HTMLSlotElement) this.#getSlotData(nodeSlot, clonedNodeSlot)
          }
  ...
  #onTemplateIdChanged (id) {
    ...
    if (!(rootNode instanceof Document) && !(rootNode instanceof ShadowRoot)) throw new Error('routes template root node is not a document or shadow root')

    this.#routesTemplate = this.#getDefaultTemplate(id, rootNode)

    this.#rootTemplate = this.#getDefaultTemplate(this.#rootId, this.#routesTemplate)
    this.#routesTemplate.content.appendChild(this.#rootTemplate).content.replaceChildren(this.#elements.slot.cloneNode())
  ...
  #setRoute (pathname, search, onSuccess) {
    ...
    else if (this.routeId) history.replaceState(history.state, '', this.routeId.concat(search))
  ...
  #setRouteTemplate (routeTemplate) {
    if (!this.#rootTemplate) throw new Error(`${elementMissing({ selectors: `template#${this.#rootId}` })} in template#${this.templateId}`)
    ...
    while (parentElement) {
      ...
      if (firstElementChild instanceof HTMLTemplateElement && firstElementChild !== routeTemplate) layoutTemplates.unshift(firstElementChild)
    ...
    if (layoutTemplates.length < this.#layouts.length) {
      for (const layout of this.#layouts.splice(layoutTemplates.length, Infinity)) {
    ...
    for (let index = 0; index < layoutTemplates.length; index++) {
      ...
      if (!(clonedHost instanceof Element)) throw new Error('host is not an Element')

      const shadowRoot = clonedHost.attachShadow({ mode: 'closed' })

      const clonedLayoutTemplate = layoutTemplate.cloneNode(true)
      if (!(clonedLayoutTemplate instanceof HTMLTemplateElement)) this.#htmlError(routeTemplate.id)

      /** @type {Layout} */
      const layout = { host: clonedHost, slots: clonedLayoutTemplate.content.querySelectorAll('slot'), template: layoutTemplate }

      if (ancestorLayout) shadowRoot.appendChild(ancestorLayout.host).replaceChildren(...clonedLayoutTemplate.content.childNodes)
      else if (clonedLayoutTemplate.content.firstElementChild instanceof HTMLSlotElement) {
        const defaultSlot = shadowRoot.appendChild(clonedLayoutTemplate.content.firstElementChild)

        defaultSlot.addEventListener('slotchange', this.#onSlotChange.bind(this))

        this.#slots.set(defaultSlot, { default: true, nodes: [this.appendChild(defaultSlot.cloneNode())], parentElement: this })
      }
