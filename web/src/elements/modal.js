...
export default class ModalElement extends CustomElement {
  ...
  connectedCallback () {
    ...
    const modalTemplate = this.#modalsTemplate?.content.getElementById(this.#anchor.pathname)?.cloneNode(true) ?? null
    if (!(modalTemplate instanceof HTMLTemplateElement)) throw new Error(`${elementMissing({ selectors: `template#${this.#anchor.pathname}` })} in template#${this.#modalsTemplate?.id}`)

    this.#modalTemplate = modalTemplate

    this.dispatch('custom-listener', { detail: { type: 'router-set', listener: this.#onRouterSet } })
    this.connected = true
  }

  /** @param {string} id */
  #onRouteIdChanged (id) {
    if (this.#anchor && this.#anchor.pathname !== id && this.#modalId && this.#modalTemplate) {
      this.#anchor.href = id.concat(this.#modalId)
      this.#modalTemplate.id = this.#anchor.pathname

      this.dispatch('router-append', { detail: { routeTemplates: [this.#modalTemplate], siblingRouteTemplateId: id, type: 'route' } })
    }
  }
  ...
  #onTemplateIdChanged (id) {
    ...
    if (!(modalsTemplate instanceof HTMLTemplateElement)) throw new Error(elementMissing({ selectors: `template#${id}` }))
  ...
  #onRouterSet (event) {
    if (this.#dialog?.open && this.#anchor?.pathname === this.routeId) this.#dialog.close() // TODO: test this better!!
    ...
    if (!this.#dialog) {
      ...
      if (!this.#dialog) throw new Error(elementMissing({ selectors: 'dialog' }))
    }
    ...
    if (!event.detail.success && event.detail.requestedId?.replace(location.search, '') === this.#modalTemplate?.id) this.dispatch('router-replace', { detail: { url: { id: 'requestedId' } } })
    else if (this.#anchor?.pathname === this.routeId) this.#dialog.showModal()
  }
}
